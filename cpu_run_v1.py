...
CPU_driver.dt = 0.05
CPU_driver.add_driver(NonLinearSolver('nonlinearsolver'))

# optim = CPU_driver.add_driver(Optimizer('optim', method='SLSQP', tol=1e-1, verbose=1))
# optim.add_child(NonLinearSolver('solver', tol=1e-1,verbose=1))  # to solve cyclic dependencies
# optim.add_unknown('heatsink.area', lower_bound=0.1, upper_bound=1.)
# optim.set_minimum('(cpu_core.T - 352)**2')
# optim.set_ 
...
#     'cpu_core.T < 81'])
# optim.add_constraints([
#     'fan_controller.tempcheck == 5'])

CPU.run_drivers()
data = CPU_driver.recorder.export_data()
print(data)
print(CPU.fan_controller.tempcheck)
